...
const server = createServer((req, res) => {
  ...
  if (url.pathname === '/list-glbs') {
    ...
    try {
      // Scan the converted/ directory tree recursively so organized
      // subfolders (monster/, npc/, character/, ...) all show up.
      const convertedDir = join(ptPortDir, 'converted');
      const glbs: string[] = [];
      ...
      if (existsSync(convertedDir)) scanDir(convertedDir, '');
      // Also include any GLBs directly in pt-port/ (legacy)
      const flat = readdirSync(ptPortDir).filter(f => f.toLowerCase().endsWith('.glb'));
      glbs.push(...flat);
      glbs.sort();
      send(res, 200, { glbs });
    } catch (e: any) {
