...
export function parseSmd(buffer: Buffer): PTSmdModel {
  ...
  // The TmFrame[32] array is always present in the file (512 bytes), but only
  // the first tmFrameCounter entries are valid. The rest are uninitialized
  // (0xCCCCCCCD debug heap fill). Always read the 512 bytes to keep the
  // stream position correct, but zero out entries beyond tmFrameCounter.
  for (let i = 0; i < 32; i++) {
    const startFrame = reader.readInt32();
    const endFrame = reader.readInt32();
    const posNum = reader.readInt32();
    const posCnt = reader.readInt32();
    if (i < tmFrameCounter) {
      tmFrames.push({ startFrame, endFrame, posNum, posCnt });
    } else {
      tmFrames.push({ startFrame: 0, endFrame: 0, posNum: 0, posCnt: 0 });
    }
  }
