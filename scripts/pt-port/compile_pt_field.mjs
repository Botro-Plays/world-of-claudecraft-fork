// Build-time compiler for Priston Tale stage SMD files.
//
// Reads a PT binary stage SMD (v0.72) and emits a generated TypeScript
// module with all geometry, UVs, materials, collision data, and the
// StageArea spatial grid needed for runtime rendering and collision.
//
// The SMD binary format is documented in scripts/pt-port/stage_smd_inspector.ts.
// This compiler extends the inspector's parsing to also read TexLinks (UVs)
// and the StageArea face lists, then filters and restructures the data for
...
// Known water material indices (from ricarten-collision-material-analysis.md).
// Mat 107 = large water surface; Mats 140 and 232 = localized water.
const KNOWN_WATER_MATS = new Set([107, 140, 232]);

// ---------------------------------------------------------------------------
// SMD Parser
// ---------------------------------------------------------------------------

function parseSmd(buf) {
...
function classifyAndBuild(smd) {
  ...
  for (let fi = 0; fi < nFace; fi++) {
    ...
    } else if (KNOWN_WATER_MATS.has(mi)) {
...
function buildTextureManifest(smd) {
  const { materials } = smd;
...
function emitModule(smd, built, uvs, textureManifest, mapName) {
  const lines = [];
  lines.push('// GENERATED by scripts/pt-port/compile_pt_field.mjs, do not edit by hand.');
  lines.push(`// Source: client/Field/${mapName.split('-')[0].charAt(0).toUpperCase() + mapName.slice(1)}/${mapName}.smd`);
...
function main() {
  const smdPath = process.argv[2] || 'client/Field/Ricarten/village-2.smd';
  const outPath = process.argv[3] || 'src/sim/pt_ricarten_field.generated.ts';
  const mapName = 'village-2';

  console.log(`Reading SMD: ${smdPath}`);
  const buf = readFileSync(resolve(smdPath));
  console.log(`  File size: ${buf.length} bytes`);

  console.log('Parsing SMD...');
  const smd = parseSmd(buf);
  console.log(`  Version: ${smd.version}`);
  console.log(`  Vertices: ${smd.nVertex}`);
  console.log(`  Faces: ${smd.nFace}`);
  console.log(`  TexLinks: ${smd.nTexLink}`);
  console.log(`  Lights: ${smd.nLight}`);
  console.log(`  Materials: ${smd.matCounter}`);
  console.log(`  Bounds: X[${smd.bounds.minX.toFixed(1)}, ${smd.bounds.maxX.toFixed(1)}] Y[${smd.bounds.minY.toFixed(1)}, ${smd.bounds.maxY.toFixed(1)}] Z[${smd.bounds.minZ.toFixed(1)}, ${smd.bounds.maxZ.toFixed(1)}]`);

  console.log('Classifying faces...');
  const built = classifyAndBuild(smd);
  console.log(`  Walkable (CHECK_FACE): ${built.nWalkable}`);
  console.log(`  Water: ${built.waterFaceIndices.length}`);
  console.log(`  Decorative: ${built.decorativeFaceIndices.length}`);

  console.log('Building UVs...');
  const uvs = buildPerFaceUVs(smd);

  console.log('Building texture manifest...');
  const textureManifest = buildTextureManifest(smd);
  console.log(`  Unique textures: ${textureManifest.length}`);

  console.log('Emitting generated module...');
  const code = emitModule(smd, built, uvs, textureManifest, mapName);
  writeFileSync(resolve(outPath), code);
  console.log(`  Written: ${outPath} (${code.length} bytes)`);
}
