...
describe('community test account transaction', () => {
  it('atomically inserts the account and all twenty level-20 character states when enabled', async () => {
    ...
    expect(characterInserts).toHaveLength(20);
    expect(new Set(characterInserts.map((call) => call[1]?.[1]))).toHaveLength(20);
  ...
  it('retries a generated name collision without aborting the transaction', async () => {
    ...
    expect(characterInserts).toHaveLength(21);
